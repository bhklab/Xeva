...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In Xeva we have implemented differnece matrix to compute PDX response.
...
The main aim of the pharmacogenomic experiemts is to find biomarkers for drug response prediction.
Xeva package provides \textbf{\textcolor{red}{drugSensitivitySig}} function to compute univariate association between PDX's moleculer data (such as gene expression) and response to a drug (gene-drug association). In the example bellow, we are computing the association between gene expression (RNASeq)
and slope of the PDXs for the drug `tamoxifen` using leanir regression (lm).
...
extended for any number of genes. For the large analysis, function also provides
out of box parall computation. User can set number of cores using the parameter
\textbf{\textcolor{red}{nthread}} in the function.

User can choose different sensitivity measures of the PDX response for the
association analysis by setting the paramert \textbf{\textcolor{red}{sensitivity.measure}}.
For example, bellow we use \textit{\textcolor{red}{best.average.response}} as
the PDXs reponse matrix in the association analysis:
<<biomarker2>>=
data(brca)
drugSensitivitySig(object=brca, drug="tamoxifen", mDataType="RNASeq",
...
For the drug-gene association analysis, user can also choose a different method
of association calcuation (such as concordance index, Pearson or Spearman
correlation) by setting the paramert \textit{\textcolor{red}{fit}}.
