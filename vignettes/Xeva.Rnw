...
\section{Replicate based PDX experiment}
Xeva can also handel replicate based experiment design. The datasets included in the package also contains replicate based PDX experimetns. To plot replicate based data:
...
Using the \textit{createXevaSet} new Xeva object can be created. The different components that are needed by the function is as fellowes-
...
\item[model]
A data.frame containig \textbf{model.id} and other relaevent
model information such as tissue type origin patient id and passage information.
All \textbf{PDXMI} variables can be inserted in this data.frame.
...
\item[drug]
A data.frame containg information about the drugs used in the experimetns.
...
\item[experiment]
A data.frame with time vs. tumore volumn infroamtion.
The required columns are \textbf{model.id}, \textbf{time}, \textbf{volume} and \textbf{drug}.
Other avaliable information such as dose amount and mouse weight can be specified as column of this data.frame .
...
\item[expDesign]
This is a R \textbf{list} object that contains the batch information.
...
This list contains all the moleculer data such as RNAseq or mutation information.
Each element of this list should contin \textbf{ExpressionSet} object.
...
It is a data.frame which contains mapping between PDX model.id and the molecularProfiles sample names.
