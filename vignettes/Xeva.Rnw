...
<<sumresB,  warning = FALSE, message=FALSE>>=
...
df <- summarizeResponse(brca, response.measure = "best.average.response", 
                        return.type = "data.frame")
df <- df[df$drug == "BYL719", ]

## add values to data.frame 
df$CDK13 <- model.type[df$patient.id]
## now plot the data 

waterfall(df, x="model.id", y="best.average.response", type="CDK13")
@
...
<<createXevaSet2>>=
xeva.set  <- setResponse(xeva.set, res.measure = c("mRECIST"), verbose=FALSE)
@
