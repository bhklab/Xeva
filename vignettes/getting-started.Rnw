...
Package can also be install directly form github using devtools:
...
<< plot1, fig.cap="Tumor growth curve for a batch of control and treated PDXs", out.width='4in', fig.wide=TRUE>>=
...
<<pdxplot2, fig.cap="Tumor growth curve for a batch of control and treated PDX. Here, the volume is normalized and plots are truncated at 40 days", out.width='4in', fig.wide=TRUE>>=
...
<<pdxplot3, fig.cap="Tumor growth curve for a batch of control and treated PDX generated using patient ID and drug name", out.width='4in', fig.wide=TRUE>>=
