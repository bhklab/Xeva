...
model.type <- if (inherits(mut, "SummarizedExperiment")) {
  assay(mut)["CDK13", ]
} else {
  Biobase::exprs(mut)["CDK13", ]
}
...
Each element of the \texttt{experiments} slot in the \textbf{MultiAssayExperiment} should contain a \textbf{SummarizedExperiment} object.
