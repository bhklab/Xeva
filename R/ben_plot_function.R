...
plotDrugResponse <- function(expSlot, expList, drugName)
{
  ## read the PDX object
  ...
  plotObject <- ggplot(plotData, aes(time, volume, group = id)) + 
    ...
    scale_color_manual(name = '',labels = c('Treatment', 'Controls'), 
                       values=c("treatment"="darkgrey", "controls" = 'black')) + 
    geom_point(aes(x = time, y = volume), size = 0.7, colour = 'darkgrey', alpha = 1) +
    xlab('Time') + 
    ylab('Volume') + 
    ggtitle(paste("Showing", n_patients, "Patients", sep = ' ')) +  
