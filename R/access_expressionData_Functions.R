...
getMolecularProfiles <- function(object, data.type) {
  ...
  if (inherits(molData, "ExpressionSet")) {
    ...
    message(sprintf(
      "Note: '%s' molecular data was stored as ExpressionSet and has been converted to SummarizedExperiment.",
      data.type
    ))
  }
...
.modelID2biobaseID <- function(
  ...
  if (nrow(modIn) == 0) {
    msg <- sprintf(
      "No model present for drug %s with molecular data type %s",
      drug,
      mDataType
    )
    if (!is.null(tissue)) {
      msg <- sprintf(
        "No model present for drug %s and tissue %s with molecular data type %s",
        drug,
        tissue,
        mDataType
      )
...
summarizeMolecularProfiles <- function(
  ...
  if (is.null(sensitivity.measure)) {
    ...
    if (is.null(senType)) {
      if (
        sensitivity.measure %in% colnames(slot(object, "sensitivity")$batch)
      ) {
        senType <- "batch"
      }
    }
  ...
  if (is.null(senType)) {
    msg <- sprintf(
      "sensitivity measure '%s' not present in model or batch sensitivity",
      sensitivity.measure
    )
    stop(msg)
  }
  ...
  if (senType == "model") {
    ...
    if (is.null(sensitivity.measure)) {
      sensitivity.measure <- colnames(sm)[
        !(colnames(sm) %in% c("model.id", "batch.name"))
      ]
    }

    modInX <- .modelID2biobaseID(
      object,
      mDataType,
      drug = drug,
      tissue = tissue,
    ...
    modIn <- modInX$data
    bioName <- modInX$bioName

    modIn[, c(sensitivity.measure)] <- sm[
      modIn$model.id,
