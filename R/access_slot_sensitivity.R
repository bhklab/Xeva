...
sensitivity <- function(
  ...
  if (is.element(type, senNames) == FALSE) {
    msg <- sprintf(
      "sensitivity 'type' can be:\n%s",
      paste(senNames, collapse = "\n")
    )
    stop(msg)
  }
  ...
  if (!is.null(sensitivity.measure)) {
    ...
    if (length(sm2take) > 0) {
      if (length(sensitivity.measure) != length(sm2take)) {
        ...
        msg <- sprintf(
          "some sensitivity.measure are not present in sensitivity slot and will be ignored\n%s\n",
          paste(notPresent, collapse = "\n")
        )
        warning(msg)
      }
      if (type == "model") {
        sm <- sm[, c("model.id", sm2take)]
      }
      if (type == "batch") {
        sm <- sm[, c("batch.name", sm2take)]
      }
    } else {
      msg <- sprintf(
        "sensitivity.measure are not present in sensitivity slot\n%s\n",
    ...
    if (is.element(type, names(slot(object, "sensitivity"))) == TRUE) {
      if (is.null(names(value))) {
        if (length(value) != nrow(slot(object, "sensitivity")[[type]])) {
          msg <- sprintf(
            "vector 'value' must be of same length as other sensitivity"
          )
          stop(msg)
        }
