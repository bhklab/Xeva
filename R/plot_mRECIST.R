...
.custom_cell_fun <- function(
  ...
  grid.polygon(
    ...
    gp = gpar(fill = c(NA, filCol), col = "#f0f0f0")
  )
}


.calculatRowColStat <- function(
  mat,
  splitBy = ";",
  scaleRow = TRUE,
  scaleCol = TRUE
...
.sortPlotMat <- function(mat, controlD, control.col, drug.col) {
  ...
  if (length(controlD[!is.na(controlD)]) > 0) {
    ...
    rwNameCol <- c(
      rep(control.col, length(controlD)),
...
plotmRECIST <- function(
  ...
  if (is.null(colPalette)) {
    ...
    colPre <- vapply(
      unqMat,
      function(x) is.null(colPalette[[x]]),
      FUN.VALUE = logical(1)
    )
    if (any(colPre) == TRUE) {
