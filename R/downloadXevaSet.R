  ...
  if (!is.null(name)) {
    if (name %in% axs$XevaSet.Name) {
      ...
      rtx <- getXevaSet(
        dw.url,
        saveDir = saveDir,
        XevaSetFileName = XevaSetFileName,
        verbose = verbose
      )
      return(rtx)
    } else {
      stop(
        "Unknown dataset name. Please use the downloadXevaSet() function for the table of available XevaSet."
      )
    }
  }
  return(axs)
